...
if ({ identifyexecutor() })[1] == "Krampus" then
    ...
    -- Use local fonts instead of downloading
    if isfile("DePixelKlein.ttf") then
        cheat.fonts["plex"].Data = readfile("DePixelKlein.ttf")
    end
    if isfile("ProggyClean.ttf") then
        cheat.fonts["mono"].Data = readfile("ProggyClean.ttf")
        cheat.fonts["spixel7"].Data = readfile("ProggyClean.ttf")
    end
end
print('load_' .. tostring(counter))
counter = counter + 1
